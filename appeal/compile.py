...
# runtime engine runs.  The engine lives in appeal/runtime.py; see
# appeal/codegen3_processor_prototype.py for the validated shapes.
#
...
def emit_source(plan):
    ...
    for slot in plan.slots:
        ...
        if slot.repeat:
            body.append(f'            self.Repeat([self.Argument({slot.name!r}, '
