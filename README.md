...
* A *nested class* decorated with `@app.command()` is a
  ...
  Its methods decorate through the class's own node.  A class
  body runs before the decorator on the class does, so fetch the
  node first--inside the class body, or just above the class;
  `app.command('db')` returns the same node either way, and the
  decorator on the class then bodies it:
